...
const SmallCard = ({ image, icon, count = 0, title = "Untitled" }: SmallCardProps) => {
  ...
  useEffect(() => {
    ...
    // Store the controls so we can cancel on cleanup
    const controls: AnimationPlaybackControls = animate(0, count, {
      duration: 2,
      ease: "easeOut",
      onUpdate: (latest) => {
        if (countRef.current) {
    ...
    // Cancel the animation when the component unmounts or count changes —
    // this stops framer-motion from writing to the DOM node after React
    // has already moved on, which was causing the UI freeze.
