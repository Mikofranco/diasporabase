...
>(({ onFilesChange, onUploadComplete, title }, ref) => {
  ...
  const handleFilesChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    selectedFiles.forEach((file) => {
      if (!isValidFileType(file)) {
        invalidMessages.push(`${file.name} — only JPG, PNG, PDF allowed`);
        return;
      }
      if (file.size > MAX_MULTIPLE_SIZE_PER_IMAGE) {
        invalidMessages.push(
          `${file.name} (${(file.size / 1024 / 1024).toFixed(1)}MB — max 5MB)`,
        );
        return;
      }
  ...
  return (
    <div className="w-full max-w-3xl mx-auto p-5 border border-border rounded-xl bg-card shadow-sm">
            ...
            {previews.map((src, index) => {
              ...
              return (
                <div key={index} className="relative group">
                  <div className="aspect-square rounded-lg overflow-hidden border border-border bg-muted/40 flex items-center justify-center">
                    {isPDF ? (
                  ...
                  <button
                    ...
                    className="absolute top-1 right-1 bg-destructive text-destructive-foreground rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold opacity-90 hover:opacity-100 transition-opacity shadow-sm"
                  >
                    ×
                  </button>
      ...
      {error && (
        <p className="mt-4 text-sm text-destructive whitespace-pre-line text-center">
          {error}
        </p>
      )}
    </div>
